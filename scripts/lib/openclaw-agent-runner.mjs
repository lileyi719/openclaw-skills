...
/** Proactive session rotate before context overflow (per turn). */
export const DEFAULT_ROTATE_MAX_SUBMITS = 5;
/** ~80 browser calls per submit (Ashby ~25, Workday ~80+); keeps browser cap aligned with submit cap. */
export const DEFAULT_ROTATE_BROWSER_PER_SUBMIT = 80;

export function defaultRotateMaxBrowser(maxSubmits = DEFAULT_ROTATE_MAX_SUBMITS) {
  return maxSubmits * DEFAULT_ROTATE_BROWSER_PER_SUBMIT;
}

export const DEFAULT_ROTATE_MAX_BROWSER = defaultRotateMaxBrowser(DEFAULT_ROTATE_MAX_SUBMITS);

export function resolveRotateMaxBrowser(rawBrowser, maxSubmits = DEFAULT_ROTATE_MAX_SUBMITS) {
  if (Number.isFinite(rawBrowser) && rawBrowser > 0) return Math.floor(rawBrowser);
  return defaultRotateMaxBrowser(maxSubmits);
}

export function checkProactiveRotateReason(stats, policy) {
  if (!policy) return null;
  const maxBrowser = policy.maxBrowserCalls ?? DEFAULT_ROTATE_MAX_BROWSER;
  if (stats.browserCalls >= maxBrowser) {
    return `browser=${stats.browserCalls}>=${maxBrowser}`;
  }
  const maxSubmits = policy.maxSessionSubmits ?? DEFAULT_ROTATE_MAX_SUBMITS;
  const sessionSubmitted = policy.getSessionSubmitted?.() ?? 0;
  if (sessionSubmitted >= maxSubmits) {
    return `session_submitted=${sessionSubmitted}>=${maxSubmits}`;
  }
  return null;
}
...
export function startSessionProgressMonitor({
  ...
  const timer = setInterval(() => {
    ...
    if (!rotateTriggered && rotatePolicy?.onThreshold) {
      const reason = checkProactiveRotateReason(cumulative, rotatePolicy);
      if (reason) {
        rotateTriggered = true;
        rotatePolicy.onThreshold(reason, { ...cumulative });
      }
    }
  }, intervalMs);

  const stop = () => clearInterval(timer);
  stop.getStats = () => ({ ...cumulative, rotateTriggered });
  return stop;
}
...
export function runOpenClawAgent({
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    let killedForProactiveRotate = false;
    let proactiveRotateReason = '';
    const startedMs = Date.now();
    const jsonlPath = sessionJsonlPath(sessionId);
    let lastJsonlSize = 0;
    ...
    const killAgent = (reason = 'proactive_rotate') => {
      if (!child || child.exitCode !== null || child.killed) return;
      killedForProactiveRotate = true;
      proactiveRotateReason = reason;
      log(`[agent] proactive rotate — stopping CLI (${reason})`);
      child.kill('SIGTERM');
    };
    onRegisterKill?.(killAgent);

    child = spawn('openclaw', agentArgv, {
      cwd: WORKSPACE,
    ...
    child.on('close', (status, signal) => {
      ...
      resolvePromise({
        ...
        signal,
        killedForIdle,
        killedForProactiveRotate,
        proactiveRotateReason,
        embeddedFallback,
      });
    });
  });
