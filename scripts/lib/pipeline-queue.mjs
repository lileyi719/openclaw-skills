...
/**
 * @param {object} job
 * @param {{ queueTier?: 'primary'|'all', allowlistOnly?: boolean, openAllowlist?: boolean, skipWorkday?: boolean }} opts
 */
export function shouldSkipJob(job, {
  ...
  if (skipWorkday && isWorkdayJob(job)) {
    return { skip: true, reason: 'workday_deferred', tier: 'workday', platform: 'workday' };
  }

  if (externalUrl) {
    const ext = shouldSkipExternalUrl(externalUrl, {
      allowlistOnly: openAllowlist ? false : allowlistOnly,
      includeSecondary: openAllowlist || queueTier !== 'primary',
      openAllowlist,
      skipWorkday,
    });
    if (ext.skip) return { skip: true, reason: ext.reason, tier: ext.tier };
    if (!openAllowlist && queueTier === 'primary' && !isPrimaryAtsUrl(externalUrl)) {
  ...
  if (openAllowlist) {
    ...
    return { skip: false, tier: c.tier, platform: c.platform };
  }
 ...
 */
export function prepareQueues(log, { queueTier = 'primary', openAllowlist = false, skipWorkday = false } = {}) {
  ...
  for (const job of externalRaw) {
    const { skip, reason } = shouldSkipJob(job, {
      queueTier: openAllowlist ? 'all' : queueTier,
      allowlistOnly: !openAllowlist,
      openAllowlist,
      skipWorkday,
    });
    if (skip) {
