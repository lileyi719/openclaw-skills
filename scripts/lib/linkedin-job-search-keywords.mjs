...
/** Strip Easy Apply filter (f_AL=true) — External Apply runs must never use it. */
export function sanitizeLinkedInSearchUrl(rawUrl) {
  if (!rawUrl || typeof rawUrl !== 'string') return rawUrl;
  try {
    const u = new URL(rawUrl.trim());
    u.searchParams.delete('f_AL');
    u.searchParams.delete('f_EA');
    return u.toString();
  } catch {
    return rawUrl.replace(/([?&])f_AL=true(&|$)/gi, '$1').replace(/([?&])f_EA=true(&|$)/gi, '$1');
  }
}
...
export function buildLinkedInJobSearchUrl(keywords, location = LINKEDIN_JOB_SEARCH_LOCATION) {
  ...
  // External Apply only — never turn on LinkedIn "Easy Apply" result filter.
  params.delete('f_AL');
  params.delete('f_EA');
  return sanitizeLinkedInSearchUrl(`https://www.linkedin.com/jobs/search/?${params.toString()}`);
}
